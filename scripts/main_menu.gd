...
var _front_menu_phase: float = 0.0
...
func _animate_front_menu(delta: float) -> void:
	_front_menu_phase += delta
	var buttons: Array = [start_btn, instr_btn, map_btn, quit_btn]
	if _skin_button != null:
		buttons.append(_skin_button)
	for i in range(buttons.size()):
		var button := buttons[i]
		if button == null:
			continue
		button.pivot_offset = button.size * 0.5
		var pulse := (sin(_front_menu_phase * 1.65 + i * 0.7) + 1.0) * 0.5
		button.scale = Vector2.ONE * (1.0 + pulse * 0.012)
		button.modulate = Color(1.0, 0.97 + pulse * 0.03, 0.92 + pulse * 0.08, 1.0)
	if _bunny_preview != null:
		_bunny_preview.pivot_offset = _bunny_preview.size * 0.5
		_bunny_preview.rotation = sin(_front_menu_phase * 1.8) * 0.08
		_bunny_preview.scale = Vector2.ONE * (1.0 + (sin(_front_menu_phase * 2.2) + 1.0) * 0.025)
